...
**Large operations split work across model sizes.** For multi-page mechanical work (frontmatter sweeps, infobox retrofits, mass renames) propose the slice split in the plan: a top-tier model for judgment-heavy slices (substantive bios, syntheses, schema), a workhorse-tier model via Agent for templated mechanical slices, and Bash for purely scripted edits. Surface this at plan time; don't default everything to the most expensive model.
...
**Plugin-sync candidate gate (plugin-sync roots only).** When the target is items under a plugin-sync root that accretes content over time (per CLAUDE.md B.2: highlights or clippings a plugin keeps adding to as the user works through a source), run two gates before ingesting, using the field and thresholds in `meta/vault-config.md`. (1) **Readiness:** include only items whose plugin-written last-updated field is at least the settling window old, so a still-growing item is not ingested half-finished; recompute from frontmatter each session rather than trusting a stale standing list. "ingest X now" overrides early; "hold X" excludes a paused one. (2) **Worthiness:** don't auto-take every settled item. Present the settled, not-yet-ingested candidates as a short table (topic · source · engagement signal · age), recommend a cut (low-engagement or off-domain items flagged as likely-skip), and let the user confirm. The engagement signal is soft, not a hard cutoff. For a batch this folds into the plan-before-run paragraph above.
...
1. **Read the inbox file.** Open its YAML frontmatter if present.
...
9. **Append to `wiki/log.md`:**
   ...
   ```
10. **Report what changed.** List every page created or modified — makes audit easy. Don't ask permission for individual entity/concept page creations; per CLAUDE.md B.5, that's the design.
...
- **Naturlig norsk per B.3** — sjekk bullets og infobox-tekst for telegram-stil, kontrastiv negasjon ("det er X, ikke Y, som ...") og latinske nominaliseringer ("kodifikator") før du logger. Disse er de hyppigste LLM-tic-ene i wiki-prosa.
