...
- **Writing & structure standard (four buckets).** All wiki prose is written to one standard; `/wiki-ingest` drafts to it and `/wiki-editor` scores against it. The four buckets are the portable pattern — the full rules and Norwegian examples live in the always-imported `meta/vaultos-lang-no.md` ("Skrive- og strukturstandard"), with a one-line-per-bucket checklist at each writing step:
  ...
  - **Prosa og setningsnivå** — mother-tongue prose, active voice, full sentences; plus the seven *naturlig norsk* anti-patterns (definitions + tests in the pack).
...
**Model-awareness (not friction).** Ingest quality is model-dependent, but most current models clear the bar (see "Ingest-recommended tier" in `meta/vault-config.md`): the top tier is best, the workhorse tier one notch down is fully usable, and only the weakest/cheapest tier collapses prose. Single-source ingest stays hands-off — no blocking model question. `wiki-ingest` stays silent on every workhorse-or-better model and flags only the case that actually hurts: running at the prose-collapse tier, where it emits a one-line note at its opening (no gate; the colleague stays in control). Batch / multi-source ingest folds model + effort confirmation into the plan-before-run pause above, rather than adding a new interruption point.
